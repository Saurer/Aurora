    ...
    public class MemoryStorage : IStorageAdapter {
        ...
        public async Task<IEnumerable<IBoxedValue>> GetPropertyContainerAttribute(int containerID, int attributeID) {
            ...
            if (boxed) {
                return await Task.WhenAll(values.Select(async value => {
                    var valueID = Int32.Parse(value);
                    var valueEvent = await GetEvent(valueID);
                    return new BoxedValue(value, valueEvent.EventValue.Value);
                }));
            }
            else {
                return values.Select(value => new BoxedValue(value, value));
            }
        }
        ...
        public async Task<IReadOnlyDictionary<int, IEnumerable<IBoxedValue>>> GetPropertyContainerAttributes(int containerID) {
            ...
            foreach (var data in attributes) {
                ...
                if (boxed) {
                    var valueID = Int32.Parse(data.Value);
                    var valueEvent = await GetEvent(valueID);
                    list.Add(new BoxedValue(data.Value, valueEvent.EventValue.Value));
                }
                else {
                    list.Add(new BoxedValue(data.Value, data.Value));
                }
            }
        ...
        public async Task<IEnumerable<IBoxedValue>> GetPropertyContainerRelation(int containerID, int relationID) {
            ...
            var values =
                ...
                select new BoxedValue(e.Value.Value, subEvent.Value.Value);
        ...
        public async Task<IReadOnlyDictionary<int, IEnumerable<IBoxedValue>>> GetPropertyContainerRelations(int containerID) {
            ...
            var relations =
                ...
                select new {
                    ID = subEvent.Value.ID,
                    Value = new BoxedValue(e.Value.Value, subEvent.Value.Value)
                };
        ...
        public async Task<bool> IsEventAncestor(int ancestor, int checkValue) {
            ...
            while (queue.Count > 0) {
                ...
                if (checkAncestor != null && eventID != checkAncestor.EventValue.BaseEventID) {
                    queue.Enqueue(checkAncestor.EventValue.BaseEventID);
                }
            }
